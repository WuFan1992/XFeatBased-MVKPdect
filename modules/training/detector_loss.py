...
def _warp_keypoints_to_other_image(kpts_wh, depth_src, depth_dst, T_src2dst, K_src, K_dst, scale_src, scale_dst, image_shape):
    if kpts_wh.numel() == 0:
    ...
    warped_dst = warped[ids] / scale_dst
    warped_dst = warped_dst.clamp(min=0)
...
class ReprojectionLocLoss(object):
    ...
    def __call__(self, pred0, pred1, correspondences):
        ...
        for idx in range(len(correspondences)):
            ...
            reprojection_errors = dist[ids0_d[:, None], ids1_d[None, :]][valid]
...
class ScoreMapRepLoss(object):
    ...
    def __call__(self, pred0, pred1, correspondences):
        ...
        for idx in range(len(correspondences)):
            ...
            pmf01_kpts = F.grid_sample(pmf01.unsqueeze(0), kpts01.view(1, 1, -1, 2), mode='bilinear', align_corners=True)[0, 0, 0, :]
            pmf10_kpts = F.grid_sample(pmf10.unsqueeze(0), kpts10.view(1, 1, -1, 2), mode='bilinear', align_corners=True)[0, 0, 0, :]
...
def compute_correspondence(model, pred0, pred1, batch, softdetect=None, radius=2, rand=True, train_gt_th=5, debug=False):
    ...
    for idx in range(b):
        ...
        kpts0_wh_ = (kpts0 / 2 + 0.5) * wh
        kpts1_wh_ = (kpts1 / 2 + 0.5) * wh
        ...
        scale0 = batch['scale0'][idx]
        scale1 = batch['scale1'][idx]

        kpts0_wh_valid, kpts01_wh, ids0, ids0_out = _warp_keypoints_to_other_image(
            kpts0_wh, depth1, depth0, T_1to0, K1, K0, scale1, scale0, wh[0]
        )
        kpts1_wh_valid, kpts10_wh, ids1, ids1_out = _warp_keypoints_to_other_image(
            kpts1_wh, depth0, depth1, T_0to1, K0, K1, scale0, scale1, wh[0]
        )
        ...
        similarity_map_01 = (desc0 @ descriptor_map1.reshape(h * w, descriptor_map1.shape[0]).t()) * 20
        similarity_map_01 = similarity_map_01.softmax(dim=-2) * similarity_map_01.softmax(dim=-1)
        similarity_map_01 = similarity_map_01.reshape(desc0.shape[0], h, w).clamp(1e-6, 1 - 1e-6)

        similarity_map_10 = (desc1 @ descriptor_map0.reshape(h * w, descriptor_map0.shape[0]).t()) * 20
        similarity_map_10 = similarity_map_10.softmax(dim=-2) * similarity_map_10.softmax(dim=-1)
        similarity_map_10 = similarity_map_10.reshape(desc1.shape[0], h, w).clamp(1e-6, 1 - 1e-6)

        similarity_map_01_valid = similarity_map_01[ids0]
        similarity_map_10_valid = similarity_map_10[ids1]

        kpts01 = 2 * kpts01_wh.detach() / wh - 1
        kpts10 = 2 * kpts10_wh.detach() / wh - 1
        ...
        correspondences.append({
            ...
            'dist_l1': dist_l1,
            'dist': dist_l2,
            'similarity_map_01': similarity_map_01,
            'similarity_map_10': similarity_map_10,
